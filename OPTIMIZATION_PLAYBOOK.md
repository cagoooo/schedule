# 優化實作手冊（OPTIMIZATION PLAYBOOK v1.2）

**配套文件**：[FUTURE_PROPOSAL.md](./FUTURE_PROPOSAL.md) │ [PROGRESS.md](./PROGRESS.md) │ [DEV_REFERENCES.md](./DEV_REFERENCES.md)  
**基準版本**：v2.41.8 │ **最後更新**：2026-04-19  
**文件目的**：針對下列 7 項提案提供「逐步實作指引、檔案修改清單、程式碼範例、驗收標準與常見陷阱」，可直接照表施工。

| # | 提案 | 預估工時 | 風險等級 | 優先 | 狀態 |
| :---: | :--- | :---: | :---: | :---: | :---: |
| 1 | 1.6 Web Push 瀏覽器通知 | 4~6 天 | 中（iOS 相容性）| 🥈 | ⏳ 待開始 |
| 2 | **1.7 ⚡ 一鍵重複預約** | 1~2 天 | 低 | 🥇 | **✅ v2.39.0 已完成** |
| 3 | **1.8 完整稽核日誌** | 3~4 天 | 低 | 🥉 | **⬆ v2.41.8 教訓提升** |
| 4 | 3.3 AI 學期白皮書 | 3~4 週 | 高（GCP 費用 + Gemini 配額）| ⭐ | ⏳ 待開始 |
| 5 | **F.1 Vitest + Playwright 測試** | 10~14 天 | 低 | 🛠 | **⬆ v2.41.x 教訓提升** |
| 6 | F.2 漸進式 TypeScript | 14~21 天 | 中（重構風險）| 🛠 | ⏳ 待開始 |
| 7 | **F.3 Sentry RUM** | 1~2 天 | 極低 | 🛠 | **🥇 強烈建議下一個** |

> **建議施工順序 (v2.41.x 後重新校準)**：~~1.7~~ → **F.3 → F.1 → 1.8** → 1.6 → F.2 → 3.3  
> 理由：v2.41.x 9 連發證明「建立觀測 + 自動化測試 + 稽核」是基石。

> [!IMPORTANT]
> **v2.41.x 教訓**：一天內 7 個 user-discovered bugs（1 個資安、2 個致命邏輯）。
> **這證明：手測絕對不夠。F.3 Sentry + F.1 自動化測試 + 1.8 稽核日誌 是接下來 3 個月最關鍵的投資。**

> [!TIP]
> **下一步建議**：F.3 Sentry RUM 僅需 1~2 天且風險極低，能立即解鎖前端 Bug 觀測能力。執行前須先到 https://sentry.io 註冊免費帳號取得 DSN。

---

# ✅ 1.7 一鍵重複預約（Quick Re-Book）— 已於 v2.39.0 完成

> **實作摘要 (2026-04-19)**：
> - **Commit**: `c2391c6`  
> - **新增函式**: `quickRebook(booking)` @ [app.js:912](../app.js#L912)
> - **修改**: `createBookingItemHTML` 新增 `showRebookBtn` 選項；歷史紀錄 + 搜尋結果同步啟用
> - **CSS**: 新增 `.btn-history-rebook` 紫藍漸層樣式 + 手機 RWD (僅顯 icon)
> - **與原計畫差異**: 採「找下個未來同星期日」演算法替代簡單 +7 天，避免落入過去日期
> - **未做的延伸（可後續加）**: 衝突時自動觸發 AI 替代方案（目前僅靠 disabled 視覺提示）
>
> 以下保留原計畫供回溯參考。
