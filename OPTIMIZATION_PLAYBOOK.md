...
| 3 | 1.8 完整稽核日誌 | 3~4 天 | 低 | 🥉 | ⏳ 待開始 |
| 4 | 3.3 AI 學期白皮書 | 3~4 週 | 高（GCP 費用 + Gemini 配額）| ⭐ | ⏳ 待開始 |
| 5 | F.1 Vitest + Playwright 測試 | 10~14 天 | 低 | 🛠 | ⏳ 待開始 |
| 6 | F.2 漸進式 TypeScript | 14~21 天 | 中（重構風險）| 🛠 | ⏳ 待開始 |
| 7 | F.3 Sentry RUM | 1~2 天 | 極低 | 🛠 | ⏳ 待開始 |

> **建議施工順序**：~~1.7~~ → **F.3 (建議下一項)** → 1.8 → F.1 → 1.6 → F.2 → 3.3  
