# 禮堂&專科教室&IPAD平板車預約系統 v2.30.0

學校禮堂、專科教室及IPAD平板車線上預約借用系統，支援查詢與登記借用時段。

## 功能特色

### 🛠️ 不開放時段管理與 UI 優化 (v2.30.0)
- **管理員自定義禁排** - 管理員可設定特定場地、特定節次為「固定不開放」
- **預約彈窗鎖定** - 禁排時段在預約選單中自動灰階並顯示鎖頭，禁止選取
- **RWD 功能按鈕群組** - 重新設計首頁按鈕佈局，實現橫向排列與智慧換行
- **系統更名全面同步** - 完整整合禮堂、專科教室與平板車的分類預約需求
- **安全性規則強化** - 針對場地設定資料實施嚴格的 Firestore 安全規則存取控制
