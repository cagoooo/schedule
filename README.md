...
### v2.48.0 (2026-04-20) - 🤖 AI 學期白皮書 (Gemini + HTML 報告 + Firebase Storage)
- 📄 **AI 自動生成學期報告**：管理員後台 → 「📄 AI 學期報告」分頁 → 一鍵產出當學期使用報告。
- 🧠 **Gemini 1.5 Flash 文案撰寫**：自動分析 10 個維度數據（總預約數、取消率、熱門場地、Top 預約者、節次分布、趨勢…），產出 4 段專業敘述（綜述 / 亮點 / 異常 / 建議）。
- 📊 **HTML 互動報告**：含 Chart.js 圖表、響應式設計、Glassmorphism 視覺，可在任何瀏覽器開啟，亦可分享連結給校長/主任。
- ☁️ **Firebase Storage 託管**：報告 HTML 上傳至 `schedule-10ed3.firebasestorage.app/reports/`，公開分享連結永久有效。
- 📜 **歷史報告列表**：所有產出的報告自動儲存於 `semesterReports` collection，後台可隨時調閱。
- 🔔 **LINE Flex 卡片推管理員**：報告產出後 1 秒內，所有訂閱告警的管理員會收到含「查看報告」按鈕的卡片。
- 🔐 **GEMINI_API_KEY Secret**：透過 Firebase Functions Secrets 安全注入，前端與 git 永遠看不到金鑰。

### v2.47.0 (2026-04-19) - 意見回饋系統 → LINE 推管理員
- 📮 **浮動意見回饋按鈕**:右下角紫色 FAB,任何老師可發起。
- 🎨 **4 種類型**:🐛 錯誤 / 💡 建議 / ❓ 問題 / 📝 其他。
- 📤 **直接 LINE 推管理員**:已訂閱告警的管理員會立即收到 Flex 卡片。
- 🔒 **防灌水**:同裝置 5 分鐘內最多 1 則。
- 💾 **完整保存**:`feedbacks` collection 永久存,管理員後台可追溯。
- 🪶 **無需登入**:任何人都能用,降低反映問題門檻。
