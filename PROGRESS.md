...
## 📅 當前版本：v2.41.7 (2026-04-19) - 搜尋日期範圍 Bug 修復 + 條件透明化
...
### 🐛 使用者困惑
> 「我已經選『跨全部場地』、節次選『第二節』、場地選『五年級IPAD車』，按搜尋預約後還是只看到禮堂的資料，請問是為什麼呢？」

### 🔍 真實原因（並非 Bug）
- 搜尋確實有跨場地（badge 顯示「跨全部場地」）
- 結果只有禮堂是因為**全資料庫中含「第二節」的預約只有 4 筆，剛好全是禮堂的畢業典禮預演**
- 五年級IPAD車目前 0 筆「第二節」預約
...
**多場地分布時**：
```
📊 場地分布：禮堂 4 筆 / 三年級IPAD車 2 筆 / 智慧教室 1 筆
```

#### 3. 視覺強化
- 計數數字放大為 1.4rem 紫色，醒目
- chips 用語意化色彩（場地紫、節次紫、關鍵字藍、期間綠）
- 分布提示用黃色 left-border banner 區隔

### 📂 修改檔案

- `app.js`: `renderSearchResults` 重寫摘要結構 + 加入 distributionHint 計算
- `app.js`: `executeAdvancedSearch` 傳遞 periodFilter 給 renderer
- `styles.v2.38.0.css`: +75 行新 chips 與 hint 樣式
