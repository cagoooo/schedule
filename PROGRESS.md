...
### 🎯 需求修正說明
原 P1-2 提案為「預約候補機制（Waitlist）」，使用者裁定**不做候補**（老師上課中難以追蹤候補狀態），改為**優化既有的「衝突時 AI 推薦替代方案」**（含 iPad 車互推其他年段）— 當下直接給可用選項，比事後候補更符合教學現場。

### 🐛 修正的核心缺陷（舊版程式碼註解自己承認的）
1. 🔴 **推薦其他場地時完全不檢查該場地的固定不開放時段**（`isPeriodUnavailableInRoom` 直接 `return false`，註「為求效能暫時忽略」）→ 老師點「立即使用」後才發現該格根本不開放。實際上 `roomSettings` 全部僅 4 個 doc，一次抓完成本極低，「效能顧慮」不成立。
2. 🔴 **換日期策略誤用主畫面場地的全域不開放設定**，modal 內換場地後判斷錯誤。
3. 換日期會推薦**週六日**（學校情境無用）。
4. iPad 車互推固定 95 分，**不考慮哪台車當天較空**。

### 📦 改良內容（[app.js](file:///h:/schedule/app.js) `findSmartAlternatives` 重寫）
1. **正確排除不開放時段**：與範圍預約查詢**並行**一次抓全部 `roomSettings` 建 `settingsMap`，三種策略（換日期／換教室／換節次）全部改用「目標場地自己的」設定判斷。
2. **空閒度智慧排序**：推薦其他場地時計算該場地**當天已借節次數**，每節 -2 分（上限 -10）— iPad 車互推自動優先「當天最空的那台」，並在卡片顯示「當天完全沒人借 👍」／「當天已借 N 節」。
3. **週末排除**：換日期策略跳過週六日。
4. **建議數 3→4**，多給老師一個選擇；移除已死的 `isPeriodUnavailableInRoom` stub。

### ✅ 驗證（本機 http server + 真實 Firestore）
- **不開放排除**：C213 週一第一節求替代 → C212/C304（週一第一節皆固定不開放）**正確不出現**（舊版會誤推）；換日期正確跳過 C213 自己不開放的週二/週四與週末，只推 7/29(三)、7/24(五)、7/31(五)。
- **空閒度排序**：學期中 2026/06/02 實測 — 六年級車該節已被借正確排除；三/四年級車 95→91 分並顯示「當天已借 2 節」。

---

## 📅 v2.54.1 (2026-07-22) - 🏫 不開放時段設定：彈窗內直接切換場地（管理員 UX）

### 🎯 使用者需求
管理員設定多個場地的不開放時段時，原本要「關閉彈窗 → 主畫面切場地 → 重開彈窗」反覆操作。現在彈窗標題列直接放場地下拉，一個視窗連續設定所有場地。

### 📦 實作內容
1. **彈窗內場地下拉**（[index.html](file:///h:/schedule/index.html) `#settingsRoomSelect`）：取代原本唯讀的場地名稱，選項**直接複製主畫面場地清單**（單一資料來源，日後場地異動不用改兩處）。
2. **狀態解耦**（[app.js](file:///h:/schedule/app.js)）：新增彈窗專屬的 `settingsCurrentRoom` / `settingsSlots`，與主畫面日曆渲染用的全域 `unavailableSlots` 完全分離 — 在彈窗內切場地**不會污染主畫面狀態**。
3. **未儲存變更保護**：切換場地時若有未存的勾選變更，先跳確認（放棄或留在原場地）。
4. **存檔不關窗**：儲存後顯示「✅ 已儲存，可繼續切換其他場地」並保持彈窗開啟；若儲存的是主畫面當前場地，同步全域狀態並重整日曆。
5. **樣式**：深綠 header 上的半透明白框下拉，展開選項深字白底。

### ✅ 驗證（本機 http server + 真實 Firestore）
- 下拉正確填入 10 個場地；切 C212 → 17 勾選（含 mon_morning）、切校史室 → 4 勾選，皆與 DB 一致。
- 未儲存變更偵測：改一格 → true、還原 → false。

---

## 📅 v2.54.0 (2026-07-22) - 📜 P1-3 預約異動履歷 + 🏆 P1-7 成就徽章
...
### 📦 實作內容
1. **Header 新增「我的預約」按鈕**（[index.html](file:///h:/schedule/index.html)）：所有使用者可見（非管理員專屬），放在 LINE 綁定按鈕前；窄螢幕自動隱藏文字只留 icon。
2. **識別方式**：依 `deviceId`（`getDeviceId()`，localStorage 持久化）查詢 —— 與自刪預約同一識別機制。`where('deviceId','==')` 為單欄位等值查詢，Firestore 自動索引，**不需複合索引**。
3. **三區分組**（[app.js](file:///h:/schedule/app.js) `loadMyBookings`）：以今日日期為界分「📅 今日 / 🔜 即將到來（未來，近到遠）/ 🗂️ 已結束（過去，新到舊，最多 30 筆）」；頂部摘要卡顯示總筆數/今日/即將到來/已結束。
4. **複用既有元件**：每筆用 `createBookingItemHTML(b, {showDeleteBtn:true, showRebookBtn:true})` 渲染，deviceId 相符即自帶「刪除」與「再預約」按鈕。
5. **刪除連動刷新**：在此彈窗刪除後自動重載清單（複用既有 historyModal 的刷新 pattern）。
6. **誠實標註**：底部註明「此清單顯示本裝置建立的預約」，換裝置/清瀏覽器資料不保留（符合 deviceId 機制的實際行為）。

### ✅ 實測驗證（本機 http server + 真實 Firestore）
- 空狀態：新裝置正確顯示「還沒有任何預約紀錄」引導。
- 有資料：以真實 deviceId `dev_1776...` 測得 9 筆全歸「已結束」區，摘要 9/0/0/9 正確，9 筆皆帶再預約+刪除按鈕（owner 判定正確）。

---

## 📅 v2.52.0 (2026-07-22) - 📱 T.2 Sentry 告警接 LINE + 🔧 修 SW 更新無限迴圈 + 🗄 L.5 資料庫健康卡 + 🧹 T.1 新學期小掃除
